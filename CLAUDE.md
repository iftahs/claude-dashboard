...
`docker:up` has a `predocker:up` hook that, on macOS only, (1) runs `scripts/sync-macos-keychain.mjs` to copy the Claude.ai OAuth token from the Keychain into `~/.claude/.dashboard-oauth-cache.json` — see `readCredentials()` below for why — and (2) runs `scripts/token-sync-agent.mjs ensure` to install a **launchd LaunchAgent** (`com.claude-dashboard.token-sync`, log at `~/Library/Logs/claude-dashboard/token-sync.log`) that re-runs the sync every 15 min. Without the agent the cache is a one-shot snapshot whose access token expires within hours and Docker live usage degrades to "OAuth token expired"; the container re-reads the cache file on every request, so a host-side re-sync heals it instantly with no rebuild. Manual controls: `npm run token-sync` (one-shot), `token-sync:install` / `token-sync:status` / `token-sync:uninstall`. All of this is a no-op on non-macOS hosts (there, Claude Code refreshes `.credentials.json` itself). **Deliberately no in-container `refresh_token` flow** — Anthropic rotates refresh tokens, and a second consumer could invalidate the host's Claude Code login.
