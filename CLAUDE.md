...
`docker:up` has a `predocker:up` hook running two host-side scripts: `scripts/sync-macos-keychain.mjs` (macOS only — copies the Claude.ai OAuth token from the Keychain into `~/.claude/.dashboard-oauth-cache.json`; see `readCredentials()` below) and `scripts/write-host-repo-dir.mjs` (all OSes — upserts `HOST_REPO_DIR=<repo path>` into `.env` so the container can render a runnable resume-watcher command on the Auto-Resume page).
