...
`docker:up` has a `predocker:up` hook (`scripts/sync-macos-keychain.mjs`) that, on macOS only, copies the Claude.ai OAuth token from the Keychain into `~/.claude/.dashboard-oauth-cache.json` before the build — see `readCredentials()` below for why. No-op elsewhere.
