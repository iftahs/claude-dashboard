...
function toolCallRecords(rows: ToolCallRow[]): ToolCallRecord[] {
...
export interface ReducedArchive {
  ...
  usageByTs: number[];
  events: { sig: string; list: UsageEvent[] } | null;
  insight: UsageRow[];
...
export function reduceArchive(files: FileRows[]): ReducedArchive {
  ...
  return {
    parts: p, usage, usageIndex, usageByTs, events: null, insight, insightOfUsage, insightBySession,
...
function archiveEvents(a: ReducedArchive, sessionPathMap: Map<string, string>): UsageEvent[] {
  if (!a.usage.length) return [];
  const sig = JSON.stringify([...sessionPathMap]);
  if (a.events?.sig !== sig) {
    a.events = { sig, list: a.usageByTs.map((i) => toEvent(a.usage[i], sessionPathMap)) };
  }
  return a.events.list;
}

/** `archive` merges as if its files were listed after `files`; it is never modified, beyond its event cache. */
export function mergeRows(files: FileRows[], sessionMetas: any[], archive: ReducedArchive = NO_ARCHIVE): MergeResult {
  ...
  const events = mergeByTs(
    liveUsage.map((r) => toEvent(r, sessionPathMap)).sort(byTs),
    archiveEvents(archive, sessionPathMap),
    (a) => usageDropped[archive.usageByTs[a]] === 1,
  );

  // ---- tool calls: distinct by tool_use id ----
  const liveCalls = toolCallRecords(parts.toolCallRows);
  ...
  const toolCalls = mergeByTs(liveCalls.sort(byTs), archive.toolCalls, (a) => callsDropped[a] === 1);
